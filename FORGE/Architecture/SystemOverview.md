...
| **FORGE Knowledge Base** | Contains the Milestone‑Driven AI Development documentation (Vision, Principles, Architecture, Standards, Development guides, AI interaction rules, Quality criteria). It serves as the reference for both human engineers and AI agents. |
...
| **CI / Tooling** | Automates verification, quality checks, and continuous delivery. It consumes only the public interfaces of packages and the FORGE guidelines, never reaching into internal implementation details. |
...
- **Reusable Packages** may reference the FORGE Knowledge Base for architectural guidance and principle alignment but remain technically independent.
- **FORGE Knowledge Base** and **Documentation** are read‑only sources for developers and AI; they guide decisions but are not compiled into the runtime.
...
- **One‑Way Dependency Flow**: Host → Packages → FORGE/Documentation. Packages never depend on higher‑level layers such as the host or CI scripts.
...
- The clear separation between **knowledge** (FORGE docs) and **code** enables AI agents to load only the relevant documentation for a task, reducing context size and improving assistance quality.
